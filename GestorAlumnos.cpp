...
#include "Cuentas.h"
    ...
    if (!font.loadFromFile("consola.ttf")) {
 	...
 	std::string s = "Administracion de Alumnos";
    ...
    sf::Text addUserTxt("Agregar Alumno", font, 18);
    ...
    sf::Text editUserTxt("Editar Alumno", font, 18);
    ...
    sf::Text delUserTxt("Eliminar Alumno", font, 18);
...
void agregarAlumno(){
	...
	sf::Text titulo("Crear Alumno", font, 24); 
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                if (boton.getGlobalBounds().contains(event.mouseButton.x, event.mouseButton.y)) {
                    if (textoInputUsuario.getString().isEmpty() || textoInputContrasena.getString().isEmpty()) {
                        ...
                        gestorCuentas.crearUsuario("ALUMNO", textoInputUsuario.getString().toAnsiString(), contrasenaReal);
                        addWindow.close();
                    }
                }
            }
...
void eliminarAlumno(){
	...
	sf::Text titulo("Eliminar Alumno", font, 24); 
	titulo.setPosition(50, 20);
	titulo.setFillColor(sf::Color::Black);
                ...
                if (boton.getGlobalBounds().contains(event.mouseButton.x, event.mouseButton.y)) {
                	...
                	for (Usuario user:usuarios){
                      	if (user.getUser()==usuarioActual && user.getTipo()==type){
                    		...
                    		textoError.setString("Usuario no es Alumno.");
						}
					}
                }
            }
